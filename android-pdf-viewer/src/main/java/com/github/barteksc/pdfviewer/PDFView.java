...
public class PDFView extends RelativeLayout {
    ...
    private List<List<PointF>> allPonitList = new ArrayList<List<PointF>>();

    private Paint pointPaint ;

    private boolean isPaintEditModel;//是否画笔编辑模式

    public boolean getPaintEditModel(){
        return isPaintEditModel;
    }

    public void setPaintEditModel(boolean model){
        isPaintEditModel = model;
    }

    public List<PointF> getPonitList(){
        return pointList;
    }

    public List<List<PointF>> getAllPonitList(){
        return allPonitList;
    }
    ...
    private void init() {
        ...
        pointPaint.setStyle(Paint.Style.STROKE);//不填充
        pointPaint.setStrokeWidth(10);  //线的宽度
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        /*for(int i=0;i<allPonitList.size();i++){
            drawPonitF(canvas,zoom,allPonitList.get(i));
        }*/
        drawPonitFs(canvas);
        drawPonitF(canvas,1,pointList);
    ...
    private void drawPonitF(Canvas canvas,float zoom,List<PointF> pointsList) {
        // Move to the target page
        float localTranslationX = 0;
        float localTranslationY = 0;
        ...
        if (swipeVertical) {
            localTranslationY = pdfFile.getPageOffset(currentPage, getZoom());
            float maxWidth = pdfFile.getMaxPageWidth();
            localTranslationX = toCurrentScale(maxWidth - size.getWidth()) / 2;
        } else {
            localTranslationX = pdfFile.getPageOffset(currentPage, getZoom());
            float maxHeight = pdfFile.getMaxPageHeight();
            localTranslationY = toCurrentScale(maxHeight - size.getHeight()) / 2;
        }
        canvas.translate(localTranslationX, localTranslationY);
      //  canvas.drawBitmap(getCacheBitmap(), 0, 0, getPointPaint());

        List<PointF> list = pointsList;
        if(list.size()>1){
            Path path = new Path();

            float mX = list.get(0).x*zoom ;
            float mY = list.get(0).y *zoom ;

            path.moveTo(mX, mY);
            for(int i=1;i < list.size();i++){
                float x = list.get(i).x*zoom ;
                float y = list.get(i).y *zoom ;
    ...
    private void drawPonitFs(Canvas canvas) {
        // Move to the target page
        float localTranslationX = 0;
        float localTranslationY = 0;
        SizeF size = pdfFile.getPageSize(currentPage);
        ...
        if (swipeVertical) {
            localTranslationY = pdfFile.getPageOffset(currentPage, zoom);
            float maxWidth = pdfFile.getMaxPageWidth();
            localTranslationX = toCurrentScale(maxWidth - size.getWidth()) / 2;
        } else {
            localTranslationX = pdfFile.getPageOffset(currentPage, zoom);
            float maxHeight = pdfFile.getMaxPageHeight();
            localTranslationY = toCurrentScale(maxHeight - size.getHeight()) / 2;
        }
        canvas.translate(localTranslationX, localTranslationY);
        //  canvas.drawBitmap(getCacheBitmap(), 0, 0, getPointPaint());
        Path path = new Path();
        for(int index = 0;index<allPonitList.size();index++) {
            List<PointF> list = allPonitList.get(index);
            if (list.size() > 1) {
                float mX = list.get(0).x * zoom;
                float mY = list.get(0).y * zoom;
    ...
    public void moveRelativeTo(float dx, float dy) {
        ...
        float pageXOffset = pdfFile.getPageOffset(page,getZoom());
        float size = pdfFile.getPageLength(page,getZoom());

        int width =getWidth();
        float tran = currentXOffset + dx;

        if (size > width*1.0){

            float gap = size - width;

            float left = -pageXOffset-gap;
            float right = -pageXOffset;

            Log.e("wizard",left+" "+right+" "+tran+ " -----"+size+" "+currentXOffset+" "+width+" "+gap+" "+pageXOffset);
